export class Board {
  ...
  placeTile(x: number, y: number, tile: string): void {
    if (x >= 0 && x < this.size && y >= 0 && y < this.size) {
      if (this.grid[y][x] === ' ' || this.grid[y][x] === tile) {
        ...
        throw new Error(
          `No. The letter ${tile} would replace the letter ${this.grid[y][x]}.`,
        )
      }
    } else {
  ...
  addWord(word: string): void {
    ...
    for (let i = 0; i < word.length; i++) {
      ...
      if (direction === 'horizontal') {
        ...
        throw new Error(
          "Invalid direction. Need to be either 'horizontal' or 'vertical'.",
        )
      }
    }
  ...
  getWordPosition(word: string): [number, number, string] {
    ...
    const commonLetter = Array.from(new Set(word.split(''))).find(letter =>
      this.lettersOnBoard.includes(letter),
    )
    if (!commonLetter) {
  ...
  getScore(): number {
    ...
    //     "Z" : 10,
    // }
    return this.lettersOnBoard.reduce(
      (score, letter) => score + letterValues[letter],
      0,
    )
  }
