      ...
      <div className="grid" style={{ gridTemplateColumns: `repeat(${maze.width}, 24px)` }}>
        {maze.grid.flat().map((tile: Tile, idx: number) => {
          ...
          return (
            <div
              key={idx}
              className={twMerge("w-[24px] h-[24px] flex items-center justify-center text-xs font-bold leading-none", bgColor, "border border-gray-800")}
            >
              {displayEmoji}
            </div>
          );
        ...
        <div className="mt-4 flex gap-2 justify-center">
          ...
          :
          <>
            <button onClick={() => movePlayer("up")} className="px-2 py-1 bg-blue-500 text-white rounded">Up (W)</button>
            <button onClick={() => movePlayer("left")} className="px-2 py-1 bg-blue-500 text-white rounded">Left (A)</button>
            <button onClick={() => movePlayer("down")} className="px-2 py-1 bg-blue-500 text-white rounded">Down (S)</button>
            <button onClick={() => movePlayer("right")} className="px-2 py-1 bg-blue-500 text-white rounded">Right (D)</button>
          </>
